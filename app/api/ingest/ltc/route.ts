...
// --- Vercel runtime hints (jednom, na vrhu) ---
...
// --- Konfiguracija preko ENV-a (s defaultima) ---
...
// ---- low-level HTTP helpers (timeout + text/json rukovanje) ----
    ...
    for (let h = start; h <= end; h++) {
      ...
      for (const txid of txids) {
        ...
        for (const vout of tx.vout || []) {
          const a = toLower(vout.scriptpubkey_address || '');
        ...
        for (const vin of tx.vin || []) {
          const a = toLower(vin.prevout?.scriptpubkey_address || '');
          if (!a || !addrMap.has(a)) continue;
      ...
      // povremeno flushaj cursor da izbjegnemo gubitak napretka
...
// (opcionalno) zaštiti GET da ne radi ništa osim što prijavi metodu
