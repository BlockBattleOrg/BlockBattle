...
// --- Tuning ---
// brži timeout po HTTP pozivu (mempool API-i su brzi; kad zablokira -> odmah fallback)
const PER_ENDPOINT_TIMEOUT_MS = Number(process.env.LTC_API_TIMEOUT_MS ?? 4000);
// koliko blokova po runu maksimalno
const MAX_HEIGHTS_PER_RUN = Math.max(1, Number(process.env.LTC_MAX_HEIGHTS ?? 10));
// za koliko blokova kaskamo od tipa kad startamo bez cursora
const START_LAG_HEIGHTS = Math.max(0, Number(process.env.LTC_START_LAG ?? 3));
// maksimalno paralelnih /tx/{txid} poziva
const TX_CONCURRENCY = Math.max(1, Number(process.env.LTC_TX_CONCURRENCY ?? 4));
// “globalni” deadline unutar Vercel limita – ostavimo buffer za cleanup/odgovor
const GLOBAL_DEADLINE_MS = Math.max(10_000, Number(process.env.LTC_GLOBAL_DEADLINE_MS ?? 50_000));
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    let lastProcessedHeight = start - 1;

    for (let h = start; h <= end; h++) {
      // globalni timeout guard
      if (Date.now() > deadlineAt) {
        await setCursor(lastProcessedHeight >= start ? lastProcessedHeight : start - 1);
        return NextResponse.json({
          ok: true,
          chain: 'LTC',
          partial: true,
          reason: 'deadline',
          processed_heights: (lastProcessedHeight - start + 1),
          tip,
          inserted,
      ...
      // limitiraj paralelizaciju fetchanja transakcija
      await mapWithConcurrency(txids, TX_CONCURRENCY, async (txid) => {
