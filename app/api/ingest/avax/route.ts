// app/api/ingest/avax/route.ts
// Avalanche C-Chain (AVAX) native transfers ingestion with multi-RPC fallback.
// - Auth via x-cron-secret === process.env.CRON_SECRET
// - Cursor in settings: key 'avax_last_block' -> { n: <blockNumber> }
// - Tracks wallets where chain IN ('AVAX','avax') AND is_active = true
...
// RPC endpoints (CSV in AVAX_RPC_URLS, else single AVAX_RPC_URL, else safe defaults)
const AVAX_RPC_URLS: string[] = (
  process.env.AVAX_RPC_URLS ||
  process.env.AVAX_RPC_URL ||
  'https://api.avax.network/ext/bc/C/rpc,https://avalanche-c-chain.publicnode.com,https://avax.meowrpc.com'
)
  .split(',')
  .map(s => s.trim())
  .filter(Boolean);
...
// Tunables
const PER_ENDPOINT_TIMEOUT_MS = Number(process.env.AVAX_RPC_TIMEOUT_MS ?? 8000);
const START_LAG_BLOCKS = Number(process.env.AVAX_START_LAG ?? 500);
const MAX_BLOCKS_PER_RUN = Number(process.env.AVAX_MAX_BLOCKS ?? 250);
const CURSOR_KEY = 'avax_last_block';
...
// AVAX C-Chain is EVM (18 decimals)
...
async function rpc<T>(method: string, params: any[] = []): Promise<T> {
  let lastErr: any;
  for (const url of AVAX_RPC_URLS) {
    try {
  ...
  throw lastErr ?? new Error('All AVAX RPC endpoints failed');
}
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!wallets.length) {
      return NextResponse.json({ ok: true, message: 'No active AVAX wallets.', inserted: 0 });
    }
    ...
    return NextResponse.json({
      ...
      rpc_endpoints_tried: AVAX_RPC_URLS.length,
    });
  } catch (err: any) {
    const msg = err?.message || String(err);
    console.error('AVAX ingest error:', msg);
