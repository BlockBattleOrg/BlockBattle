...
function normChain(x?: string | null) {
  const s = String(x || "").trim().toUpperCase().replace(/[\s_-]+/g, "");
  return s; // e.g. "ARBITRUMONE"
}
function isARB(symbol?: string | null, chain?: string | null) {
  const s = String(symbol || "").trim().toUpperCase();
  const c = normChain(chain);
  return s === "ARB" || c === "ARB" || c === "ARBITRUM" || c === "ARBITRUMONE";
}
...
export async function POST(req: Request) {
  try {
    ...
    // 1) aktivni ARB wallet-i (tolerantno na missing currency_id)
    ...
    const wallets = (walletsAll || []).filter((w: any) => {
      ...
      return w?.address && isARB(cur?.symbol, w?.chain);
    });

    if (wallets.length === 0) {
      return NextResponse.json({ ok: true, info: "No ARB wallets active; nothing to ingest.", inserted: 0 });
    }
    ...
    const KEY_MARKER = "arb_contrib_last_scanned";
    ...
    return NextResponse.json({ ok: false, error: e?.message || "ARB contrib ingest failed" }, { status: 500 });
  }
}
