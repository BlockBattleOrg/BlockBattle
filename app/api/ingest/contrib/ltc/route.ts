...
/**
 * LTC (Litecoin) native contributions ingest (UTXO).
 ...
 * - Cursor is stored in `public.settings` under key `ltc_contrib_last_scanned`.
 *
 * ENV:
 *  - CRON_SECRET
 ...
 *  - LTC_RPC_POOL   (optional; comma/newline-separated list of base URLs)
 *  - LTC_RPC_URL    (fallback single base, e.g. https://ltc.nownodes.io)
 ...
 *  - sinceHours=<n>               // time-based backoff (≈ 150s per block)
...
function parsePool(envPool?: string, envSingle?: string) {
  ...
  push(envSingle);
  if (process.env.LTC_PUBLIC_RPC_URL) push(process.env.LTC_PUBLIC_RPC_URL);
...
async function ltcRpc<T = any>(
  ...
  for (let i = 0; i < retries; i++) {
    await sleep(400 * (i + 1));
    const { result, used } = await ltcRpc<T>(method, params, pool, apiKey, { retries: 0, timeoutMs, debugLog });
    return { result, used };
  }
...
// ---------- Helpers ----------
const unitToAmount = (v: number | bigint) => Number(v) / 10 ** DECIMALS;
...
export async function POST(req: Request) {
  ...
  try {
    ...
    // Load wallets and filter to LTC
    ...
    const funding = (wallets || [])
      .filter(w => canonChain(String(w.chain)) === "LTC")
      .map(w => ({ id: String(w.id), addr: String((w as any).address || "").toLowerCase() }))
      .filter(w => !!w.addr);

    if (!funding.length) {
      return ok({ note: "No active LTC wallets", inserted: 0, scanned: 0 });
    }
    ...
    // RPC pool
    const pool = parsePool(process.env.LTC_RPC_POOL, process.env.LTC_RPC_URL || "https://ltc.nownodes.io");
    ...
    // Tip & safe head
    const tipCount = await ltcRpc<number>("getblockcount", [], pool, apiKey, { debugLog: dbg });
    const tip = tipCount.result;
    const safeTip = Math.max(0, tip - minConf);

    // Force single tx
    if (forceTx) {
      const txRes = await ltcRpc<any>("getrawtransaction", [forceTx, true], pool, apiKey, { debugLog: dbg });
      ...
      if (bhash) {
        const blk = await ltcRpc<any>("getblock", [bhash, 1], pool, apiKey, { debugLog: dbg });
      ...
      for (const v of vouts) {
        ...
        if (m) {
          ...
          const valueLtc = Number(v?.value || 0);
          matches.push({ wallet_id, amount: valueLtc });
        }
      }
    ...
    for (let h = from; h <= to; h++) {
      const bh = await ltcRpc<string>("getblockhash", [h], pool, apiKey, { debugLog: dbg });
      const blk = await ltcRpc<any>("getblock", [bh.result, 2], pool, apiKey, { debugLog: dbg });
      scanned++;
      ...
      for (const tx of txs) {
        ...
        for (const v of vouts) {
          ...
          for (const a of addrs) {
            ...
            const val = Number(v?.value || 0); // value in LTC
