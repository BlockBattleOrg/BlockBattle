...
function weiToNumber(wei: Hex | string, decimals = 18): number {
  const s = weiToDecimalString(wei, decimals);
  return Number(s);
}
...
async function getFundingAddresses(client: SupabaseClient): Promise<Set<string>> {
  // Kompatibilno s aliasima: pol/matic/polygon
...
export async function POST(req: Request) {
  try {
    ...
    const forceTx = url.searchParams.get("tx"); // direktan debug insert
    const traceInternal = (process.env.TRACE_INTERNAL || "").toLowerCase() === "true";

    // ⚠️ privremena/diagnostička konverzija: USD po 1 POL (npr. ?usdPerPol=0.48)
    ...
    // ---- Direktni TX put (debug / internal) ----
    if (forceTx) {
      ...
      const nativeAmount = toBig(tx.value);

      let captured = addrs.has(to) && nativeAmount > 0n;
      ...
      // USD iznos (privremeno: native * usdPerPol ako je zadan)
      const polAmount = weiToNumber(tx.value);
      const amount_usd = useUsd ? polAmount * useUsd : null;

      const row: Record<string, any> = {
        chain: "pol",
        symbol: "POL",
        from_address: from,
        to_address: addrs.has(to) ? to : null,
        tx_hash: tx.hash,
        block_number: blk,
        block_time: new Date(ts * 1000).toISOString(),
        amount_usd: amount_usd, // može biti null; normalizator kasnije može popuniti
      };
      ...
      return NextResponse.json({ ok: true, inserted: 1, tx: tx.hash, block: blk, reason: "forceTx", usdPerPol: useUsd || undefined });
    }
    ...
    while (cursor <= limitTo) {
      ...
      for (const blk of blocks) {
        ...
        for (const tx of blk.transactions || []) {
          const to = lc(tx.to || "");
          if (to && addrs.has(to) && toBig(tx.value) > 0n) {
            const polAmount = weiToNumber(tx.value);
            const amount_usd = useUsd ? polAmount * useUsd : null;
            ...
            rows.push({
              ...
              block_number: bn,
              block_time: new Date(ts * 1000).toISOString(),
              amount_usd, // null ako nema usdPerPol; može se kasnije popuniti normalizatorom
            });
          }
        }
      ...
      if (!maxBlocks) break; // jedan batch default, cron-friendly
    }
