...
/**
 * AVAX (Avalanche C-Chain) native contributions ingest (EVM).
 ...
 * - Cursor (last scanned block) is stored in `public.settings` under `avax_contrib_last_scanned`.
 *
 * ENV required:
 *  - CRON_SECRET
 ...
 *  - AVAX_RPC_POOL   (optional, comma/newline-separated list of base URLs)
 *  - AVAX_RPC_URL    (fallback single base, e.g. https://avax.nownodes.io)
...
// ---------- Tunables & helpers ----------
const DECIMALS = 18;              // AVAX native uses 18 decimals
...
const CURSOR_KEY = "avax_contrib_last_scanned";
...
function parsePool(envPool?: string, envSingle?: string) {
  ...
  // Minimal last-resort (only if provided by user in env)
  if (process.env.AVAX_PUBLIC_RPC_URL) push(process.env.AVAX_PUBLIC_RPC_URL);
...
export async function POST(req: Request) {
  ...
  try {
    ...
    // Load wallets and filter to AVAX via canonChain
    ...
    const funding = (wallets || [])
      .filter(w => canonChain(String(w.chain)) === "AVAX")
      .map(w => ({ id: String(w.id), addr: String((w as any).address || "").toLowerCase() }))
      .filter(w => !!w.addr);

    if (!funding.length) {
      return ok({ note: "No active AVAX wallets", inserted: 0, scanned: 0 });
    }
    ...
    // RPC pool
    const pool = parsePool(process.env.AVAX_RPC_POOL, process.env.AVAX_RPC_URL || "https://avax.nownodes.io");
