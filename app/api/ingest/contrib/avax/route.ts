// app/api/ingest/contrib/avax/route.ts
...
const RPC = (process.env.AVAX_RPC_URL || "").trim();
...
type Hex = string;
type EvmTx = { hash: string; to: string | null; value: Hex };
type EvmBlock = { number: Hex; timestamp: Hex; transactions: EvmTx[] };
...
async function rpc<T>(method: string, params: any[]): Promise<T> {
  if (!RPC) throw new Error("Missing AVAX_RPC_URL");
  if (!/\/ext\/bc\/C\/rpc(\b|\/|$)/i.test(RPC)) {
    throw new Error(
      "AVAX_RPC_URL must point to the C-Chain JSON-RPC endpoint (…/ext/bc/C/rpc)."
    );
  }
...
// Supabase relation may come as object OR array
...
function isAVAX(symbol?: string | null, chain?: string | null) {
  const s = String(symbol || "").trim().toUpperCase(); // 'AVAX'
  const c = normChain(chain); // 'AVALANCHE', 'AVALANCHEC', 'CCHAIN'
  return s === "AVAX" || c === "AVAX" || c === "AVALANCHE" || c === "AVALANCHEC" || c === "CCHAIN";
}
...
export async function POST(req: Request) {
  try {
    ...
    // 1) Active AVAX wallets (tolerant to missing currency_id)
    ...
    const wallets = (walletsAll || []).filter((w: any) => {
      ...
      return w?.address && isAVAX(cur?.symbol, w?.chain);
    });

    if (wallets.length === 0) {
      return NextResponse.json({ ok: true, info: "No AVAX wallets active; nothing to ingest.", inserted: 0 });
    }
    ...
    const KEY_MARKER = "avax_contrib_last_scanned";
    ...
    // 3) Scan
    const matches: Array<{ wallet_id: string; hash: string; tsMs: number; amount: number }> = [];
    ...
    return NextResponse.json({ ok: false, error: e?.message || "AVAX contrib ingest failed" }, { status: 500 });
  }
}
