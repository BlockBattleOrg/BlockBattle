// app/api/ingest/op/route.ts
// Optimism (OP) native transfers ingestion with multi-RPC fallback.
// Same pattern as ETH: x-cron-secret, cursor in settings, idempotent inserts.

import { NextRequest, NextResponse } from 'next/server';
import { getSupabaseAdmin } from '@/lib/supabaseAdmin';
const supabase = getSupabaseAdmin();

const OP_RPC_URLS: string[] = (
  process.env.OP_RPC_URLS ||
  process.env.OP_RPC_URL ||
  'https://optimism.publicnode.com,https://optimism.llamarpc.com,https://mainnet.optimism.io'
).split(',').map(s => s.trim()).filter(Boolean);

const PER_ENDPOINT_TIMEOUT_MS = Number(process.env.OP_RPC_TIMEOUT_MS ?? 8000);
const START_LAG_BLOCKS = Number(process.env.OP_START_LAG ?? 500);
const MAX_BLOCKS_PER_RUN = Number(process.env.OP_MAX_BLOCKS ?? 250);
const CURSOR_KEY = 'op_last_block';
...
async function rpc<T>(method: string, params: any[]=[]): Promise<T> {
  let last:any; for (const url of OP_RPC_URLS) { try { return await rpcAttempt<T>(url,method,params); } catch(e){ last=e; } }
  throw last ?? new Error('All OP RPC endpoints failed');
}

async function getActiveWallets() {
  const { data, error } = await supabase.from('wallets').select('id,address')
    .in('chain', ['OP','op','optimism']).eq('is_active', true);
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!wallets.length) return NextResponse.json({ ok:true, message:'No active OP wallets.', inserted:0 });
    ...
    await setCursor(end);
    return NextResponse.json({ ok:true, chain:'OP', range:{ start, end }, latest, blocks:end-start+1, inserted, time_window:{ firstTs, lastTs }, rpc_endpoints_tried: OP_RPC_URLS.length });
  } catch (err:any) {
    const msg = err?.message || String(err); console.error('OP ingest error:', msg);
