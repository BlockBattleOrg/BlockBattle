// app/api/claim-ingest/pol/route.ts
// Polygon (PoS / POL) claim-ingest aligned to stable ETH/AVAX model (chain='pol')
...
const POL_RPC_URL = process.env.POL_RPC_URL || ''; // matic.nownodes.io
...
function hexToBigInt(hex: string): bigint { const clean = hex.startsWith('0x') ? hex.slice(2) : hex; return BigInt('0x' + (clean || '0')); }
...
function weiToEthString(weiHexOrDec: string): string {
  ...
  const ether = wei / 10n ** 18n; const remainder = wei % 10n ** 18n;
...
async function rpc(method: string, params: any[]) {
  if (!POL_RPC_URL) return { ok: false as const, error: 'missing_pol_rpc_url' };
  const body = JSON.stringify({ jsonrpc: '2.0', id: 1, method, params });
  try {
    const headers: Record<string, string> = { 'content-type': 'application/json' };
    if (NOWNODES_API_KEY) { headers['api-key'] = NOWNODES_API_KEY; headers['x-api-key'] = NOWNODES_API_KEY; }
    const res = await fetch(POL_RPC_URL, { method: 'POST', headers, body, cache: 'no-store' });
...
// USD pricing (kept ETH-based best-effort)
...
// DB helpers
async function findPolWalletIdByToAddress(addr: string): Promise<string | null> {
  const addrLower = String(addr || '').toLowerCase();
  const addrNo0x = lowerNo0x(addrLower);
  const sel = 'id,address,chain';

  { const { data } = await supa().from('wallets').select(sel).eq('chain','pol').eq('address', addrLower).limit(1);
    if (data && data.length) return String(data[0].id); }
  { const { data } = await supa().from('wallets').select(sel).eq('chain','pol').ilike('address', addrLower).limit(1);
    if (data && data.length) return String(data[0].id); }
  { const { data } = await supa().from('wallets').select(sel).eq('chain','pol')
...
export async function POST(req: NextRequest) {
  ...
  const wallet_id = toAddress ? await findPolWalletIdByToAddress(toAddress) : null;
  if (!wallet_id) {
