...
// Arbitrum claim-ingest aligned to repo schema (mirrors ETH route):
// - contributions: wallet_id, tx_hash, amount, amount_usd, block_time, priced_at, note
// - wallets: id, address, chain
// Mapping rule: wallet = wallets.address == tx.to (case-insensitive) AND chain == 'arb'
...
// For Arbitrum we first try ARB_RPC_URL; fallback to generic EVM_RPC_URL if you prefer routing per deployment.
const ARB_RPC_URL =
  process.env.ARB_RPC_URL ||
  process.env.EVM_RPC_URL ||
  'https://arb.nownodes.io';
...
async function rpc(method: string, params: any[]) {
  if (!ARB_RPC_URL) return { ok: false as const, error: 'missing_rpc_url' };
...
async function findArbWalletIdByToAddress(addr: string): Promise<string | null> {
  // match address + chain=='arb', cover variants without 0x
  ...
  // 1) eq + chain
  {
    const { data, error } = await supa()
      .from('wallets').select(sel).eq('chain', 'arb').eq('address', addrLower).limit(1);
  ...
  {
    const { data, error } = await supa()
      .from('wallets').select(sel).eq('chain', 'arb').ilike('address', addrLower).limit(1);
    if (!error && data && data.length) return String(data[0].id);
  }
  ...
  {
    const { data, error } = await supa()
      .from('wallets').select(sel).eq('chain', 'arb')
...
export async function POST(req: NextRequest) {
  ...
  const note = (noteFromBody || noteFromQuery || '').toString().trim() || null;
  ...
  // map wallet_id by address + chain='arb' (must be our project wallet)
