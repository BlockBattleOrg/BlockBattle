// app/api/claim-ingest/ltc/route.ts
// Litecoin claim-ingest aligned to stable BTC/ETH model (same response contract)
//
// Tables:
// - contributions: wallet_id, tx_hash, amount, amount_usd, block_time, priced_at, note
// - wallets: id, address, chain
// Mapping: sum vout.value for outputs whose address matches wallets.address where chain='ltc'
// Pricing: best-effort via lib/fx (symbol: 'LTC')
// Idempotent on tx_hash
//
// ENV used: LTC_RPC_URL, NOWNODES_API_KEY, SUPABASE_SERVICE_ROLE_KEY, NEXT_PUBLIC_SUPABASE_URL, CRON_SECRET
...
const LTC_RPC_URL = process.env.LTC_RPC_URL || '';
...
async function priceToUsdOrNull(symbol: 'LTC', amountStr: string): Promise<{ usd: number; pricedAt: string } | null> {
  try {
...
async function rpc(method: string, params: any[]): Promise<RpcOk | RpcErr> {
  if (!LTC_RPC_URL) return { ok: false, error: 'missing_ltc_rpc_url' };
  const body = JSON.stringify({ jsonrpc: '2.0', id: 1, method, params });
  try {
    const headers: Record<string, string> = { 'content-type': 'application/json' };
    if (NOWNODES_API_KEY) { headers['api-key'] = NOWNODES_API_KEY; headers['x-api-key'] = NOWNODES_API_KEY; }
    const res = await fetch(LTC_RPC_URL, { method: 'POST', headers, body, cache: 'no-store' });
...
function extractAddresses(spk: any): string[] {
  // Common fields across Litecoin full nodes
  const out: string[] = [];
...
// -------- DB helpers --------
async function findLtcWalletIds(addrs: string[]): Promise<Record<string, string>> {
  if (!addrs.length) return {};
  const { data } = await supa().from('wallets').select('id,address,chain').eq('chain','ltc').in('address', addrs);
...
export async function POST(req: NextRequest) {
  ...
  if (v.ok && v.result) {
    ...
    tx = d.result;
    confirmations = 0; // no block info from pure decode
  }
  ...
  const addrToWalletId = await findLtcWalletIds(outAddresses);
  const ourAddrs = Object.keys(addrToWalletId);
  if (!ourAddrs.length) {
    return json(200, { ok:false, code:'not_project_wallet', message:'The transaction is not directed to our project. The wallet address does not belong to this project.', data:{ txHash: txid } });
  }

  // sum outputs that go to our addresses
  let total = 0;
  ...
  const priced = await priceToUsdOrNull('LTC', amountStr);
